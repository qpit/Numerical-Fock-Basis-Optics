...
def _secretKey(M,N,dim):
    # Computes the secret key of a density matrix M composed of 2 subsystems each with N qudits, with each qudit of dimension d
    
    idntty = np.identity(dim**N)
    L = list(itertools.product(np.arange(0,dim), repeat=N))
    
    # Compute the mutual information
    PAB = np.zeros((dim**N,dim**N))
    for i in range(0,len(L)):
        for j in range(0,len(L)):
            x = list(L[i])
            y = list(L[j])
            ProjA = np.kron(_altBasis(x),_altBasis(x).reshape(1,-1))#np.kron(_buildKet(x),_buildKet(x).reshape(1,-1))
            ProjB = np.kron(_altBasis(y),_altBasis(y).reshape(1,-1))
            Proj = np.kron(ProjA,ProjB)
            PAB[i,j] = np.real(np.trace(Proj.dot(M)))
    ...
    for i in range(0,len(L)):
        for j in range(0,len(L)):
            if PAB[i,j] != 0:
                MI = MI + PAB[i,j]*np.log2(PAB[i,j]/PA[i]/PB[j])
    
    # Compute the Holevo bound
    HO =  _vonNeumannEntropy(M)   
    for i in range(0,len(L)):
        x = list(L[i])
        ProjA = np.kron(_altBasis(x),_altBasis(x).reshape(1,-1))
...
def _bellCorr(rho):
    ...
    va00 = np.array([1,0])
    va01 = np.array([0,1])
    va10 = (np.array([1,0]) + np.array([0,1]))/np.sqrt(2)
    va11 = (np.array([1,0]) - np.array([0,1]))/np.sqrt(2)
    
    vb00 = np.cos(np.pi/8)*np.array([1,0])+np.sin(np.pi/8)*np.array([0,1])
    vb01 = np.sin(-np.pi/8)*np.array([1,0])+np.cos(-np.pi/8)*np.array([0,1])
    vb10 = np.cos(-np.pi/8)*np.array([1,0])+np.sin(-np.pi/8)*np.array([0,1])
    vb11 = np.sin(np.pi/8)*np.array([1,0])+np.cos(np.pi/8)*np.array([0,1])

    ma0 = (-np.kron(va00.reshape(-1,1),va00)
                + np.kron(va01.reshape(-1,1),va01))
    ma1 = (-np.kron(va10.reshape(-1,1),va10) 
                + np.kron(va11.reshape(-1,1),va11))
    
    mb0 = (-np.kron(vb00.reshape(-1,1),vb00)
                + np.kron(vb01.reshape(-1,1),vb01))
    mb1 = (-np.kron(vb10.reshape(-1,1),vb10) 
                + np.kron(vb11.reshape(-1,1),vb11))
    
    corrArray = np.array([
        [expVal(rho,ma0,mb0),expVal(rho,ma0,mb1)],
        [expVal(rho,ma1,mb0),expVal(rho,ma1,mb1)]
        ])
...
def _getCurve(D,l):
    ...
    for jj in range(0,pairs): # jj is the number of swaps
        ...
        rho,P = _createPair(etaR,etaL,gamma,Pd,CT,avg,thetaR,thetaL)#_rho(N,avg,phi,etaR,etaL,thetaR,thetaL)
        ...
        F[jj] = f
        Y[jj] = np.real(_secretKey(rhoSwap,N,2)*f) # Computes the secret key rate, normalized by darkAtt
        X[jj] = (jj+1)*l # Vector of repeater locations (distance along x)
        avgs[jj] = avg
        thetaLs[jj] = thetaL
        thetaRs[jj] = thetaR
    return X,Y,wwVals,F,avgs,thetaLs,thetaRs,rhoSwap

def _testSwap(s,thetaR):
    
    rho = np.array([
        [1,0,0,(-np.exp(-1j*phi))**(s+1)],
        [0,(s+1)*(1/etaR-1)*np.tan(thetaR)**2,0,0],
        [0,0,0,0],
        [(-np.exp(-1j*phi))**(s+1),0,0,1]       
        ])
...
darkAtt = 500# Average number of entanglement sharing attempts allowed by the experimenter
nAvgs = 15 # Curve averages
NT = 10 # Number of terms in the sum described by eq 11
...
l = 60 # Distance between repeaters
D = 400 # Total distance
etaR = 10**(-a*l/10) # Channel transmitivity for each repeater
...
varList = [0,0.5*10**(-4),1*10**(-4),1.5*10**(-4)]
for nn in range(0,len(varList)):
    ...
    for mm in range(0,nAvgs):
        X,Y,wwVals,F,avgs,thetaLs,thetaRs,rhoSwap = _getCurve(D,l)
        Yavg = Yavg + Y
        wwAvg = wwAvg + wwVals
    Yavg = Yavg/nAvgs
    wwAvg = wwAvg/nAvgs
...
plt.legend([r'0',r'$0.5\cdot10^{-4}$',r'$1.0\cdot10^{-4}$',r'$1.5\cdot10^{-4}$'])
...
plt.grid(1)
plt.savefig('secretKeyPd60km500Att',dpi=400)
